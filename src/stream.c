...
static enum StreamErrorCode fail(struct StreamRange* range, enum StreamErrorCode error)
{
        range->error = error;
        range->next = next_zeros;
...
void stream_on_memory(struct StreamRange* range, uint8_t const* mem, size_t const size)
{
        range->start = mem;
        range->cursor = mem;
